...
@section('content')
	...
	<div class="d-flex flex-wrap justify-content-between justify-align-center">
		...
		<div class="my-3">
			<button type="button" class="btn btn-indigo" data-toggle="modal" data-target="#novoproduto"><i class="fas fa-plus"></i></button> 
		</div>
	</div>
					...
					@if(count($produtos) == 0)
						<tr>
							<td colspan="6" class="text-center">
								<button type="button" class="btn btn-link" data-toggle="modal" data-target="#novoproduto"> Adicione um produto</button> 
							</td>
						</tr>
					@endif
					...
					<tr>
						...
						<td>
							<form method="post" action="{{route('produtos.destroy', $prod->id)}}">
								<button type="button" onclick="editar({{$prod->id}}, '{{$prod->nome}}', '{{$prod->preco}}', '{{$prod->estoque}}', '{{$prod->categoria_id}}')" class="btn btn-sm"><i class="fas fa-edit fa-lg text-indigo"></i></button>
								@csrf
								@method('DELETE')
								<button type="submit" class="btn btn-sm"><i class="fas fa-trash-alt fa-lg text-danger"></i></button>
							</form>
						</td>
					</tr>
					@endforeach
    ...
    <div class="modal fade" id="novoproduto" tabindex="-1" role="dialog" aria-labelledby="AddProdutoModalLabel" aria-hidden="true">
    ...
    <div class="modal fade" id="editarproduto" tabindex="-1" role="dialog" aria-labelledby="EditarProdutoModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
            	<form method="post" action="{{route('produtos.update')}}">
	                ...
	                <div class="modal-body row">
						...
						<div class="form-group col-12">
							...
							<select id="editar_categoria" name="editar_categoria" class="custom-select {{$errors->has('editar_categoria')?'is-invalid':''}}">
								<option>-- Selecione uma categoria --</option>
								@foreach($categorias as $cat)
